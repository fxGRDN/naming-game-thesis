...
def consensus_threshold(sampling_freq=100):
    ...
    for stat in range(4):
        ...
        if stat == 1:
            contours = measure.find_contours(Z, level=0.90)
            if contours:
                longest_contour = max(contours, key=len)
                contour_x = np.interp(longest_contour[:, 1], np.arange(Z.shape[1]), bit_flip_prob)
                contour_y = np.interp(longest_contour[:, 0], np.arange(Z.shape[0]), sample_idx)

                f = lambda x, a, b, c, d: np.exp(a*x + b) + c*x**2 + d

                popt, _ = curve_fit(f, contour_x, contour_y, p0=(1, 1, 1, 1), maxfev=10000)
                fitted_curve = f(contour_x, *popt)

                axhm.plot(contour_x, fitted_curve, color='red', linewidth=1, linestyle='dashed', alpha=0.7, label='Dopasowana krzywa')
                print(f'Fitted curve parameters for 90% consensus: {popt}')
            axhm.legend()
            cs = axhm.contour(Y, X, Z, levels=[0.90], colors='black', linewidths=1, linestyles='dashed', alpha=0.7)
...
def print_latex_table_multiple(param_values, sampling_freq=100):
    ...
    print("\\centering")
    print("\\caption{Statystyki symulacji vs szansa pomylenia słowa}")
    print("\\label{tab:word_confusion_stats}")
